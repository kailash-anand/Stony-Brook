...
public class HigherOrderUtils 
{
    ...
    public static NamedBiFunction<Double, Double, Double> substract = new NamedBiFunction<Double,Double,Double>() 
    {
    ...
    public static NamedBiFunction<Double, Double, Double> divide = new NamedBiFunction<Double,Double,Double>() 
    {
        ...
        @Override
        public Double apply(Double first, Double second) throws ArithmeticException {
            if(second == 0)
            {
    ...
    */
    public static <T> T zip(List<T> args, List<? extends BiFunction<T, T, T>> bifunctions)
    {
        ...
        {
            throw new IllegalArgumentException("Number of arguments and bifunctions do not match");
        }
        
        IntStream.range(0, args.size()-1).forEach(
            i -> args.set((i+1),(bifunctions.get(i).apply(args.get(i), args.get(i+1)))));
